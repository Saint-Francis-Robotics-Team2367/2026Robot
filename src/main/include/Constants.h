...
static constexpr double indexerStallCurrent =
    35.0;  // amps — tune based on observed stall current
static constexpr double indexerStallVelocityThreshold =
    0.5;  // turns/sec — near-zero means stalled
}  // namespace IndexerConstants
...
// Hub target position on the field (inches, origin = back-left, X = right, Y =
// forward)
inline static double hubX = 182.11;  // from field diagram left-wall measurement
inline static double hubY =
    158.85;  // (317.69 / 2) confirmed by 158.84 dims each side
...
constexpr double turretPulleyRatio =
    44;  // big wheel to small wheel (encoder) ratio
