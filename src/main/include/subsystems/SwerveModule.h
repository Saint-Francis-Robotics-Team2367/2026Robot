...
class SwerveModule {
 private:
  ...
  ctre::phoenix6::controls::VelocityVoltage velocityVoltage{
      0_tps};  // turns per second
 ...
 public:
  ...
  enum control { VELOCITY, POSITION };

  // motor can either be STEER or DRIVE
  enum MotorType { STEER, DRIVE };

  SwerveModule(int driveMotorID, int steerMotorID, int encoderID,
               std::string canBus = "rio");  // canBus can also be canivore

  void setMotor(control controlType, MotorType motorType, double input);
  void stopModule();
  void initHardware();
  void invertModule(ctre::phoenix6::signals::InvertedValue value, bool steer,
                    bool drive);  // Defaulted to CounterClockwise Positive
