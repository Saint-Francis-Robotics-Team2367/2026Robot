...
frc2::CommandPtr RunIntake::IntakeCommand(RunIntake* intake, double speed) {
  return frc2::cmd::StartEnd([intake, speed] { intake->setMotorSpeed(speed); },
                             [intake] { intake->stop(); }, {intake});
}

void RunIntake::setMotorSpeed(double speed) {
  rollerMotor.SetControl(ctre::phoenix6::controls::VelocityVoltage{
      units::turns_per_second_t(speed / 60.0)}
                             .WithSlot(0));
}
