...
void Shooter::moveHoodToZero() {
  ...
  RackMotor.SetControl(
      ctre::phoenix6::controls::PositionDutyCycle{units::angle::turn_t{0.0}});
}
...
void Shooter::init() {
  ...
  RackConfig.MotorOutput.NeutralMode =
      ctre::phoenix6::signals::NeutralModeValue::Brake;
  RackConfig.MotorOutput.Inverted =
...
bool Shooter::setFlywheelSpeed(float shooterRPM) {
  ...
  ShooterMotor.SetControl(ctre::phoenix6::controls::VelocityVoltage{
      units::angular_velocity::turns_per_second_t{efficientRPM / 60.0}});
...
void Shooter::setHoodPosition(float shooterRPM, float horizontalOffset,
  ...
  float dx =
      (1 * ShooterConstants::InchesToMeters) +
      std::sqrt((horizontalOffset * ShooterConstants::InchesToMeters) *
                    (horizontalOffset * ShooterConstants::InchesToMeters) +
                (yOffset * ShooterConstants::InchesToMeters) *
                    (yOffset * ShooterConstants::InchesToMeters));
  frc::SmartDashboard::PutNumber("Shooter Distance (dx)", dx);
  const float dy = 72.0f * ShooterConstants::InchesToMeters;
  const float verticalOffset =
      dy - (shooterHeight * ShooterConstants::InchesToMeters);

  // Desired vertex location
  // Alter (24) to make it shooter higher
  // const float VertexYPose = verticalOffset + (24.0f *
  // ShooterConstants::InchesToMeters); float VertexXPose = dx - (24.0755062252f
  // * ShooterConstants::InchesToMeters);

  // Solve projectile equation at (dx, dy)
  const float A =
      (ShooterConstants::GRAVITY * dx * dx) / (2.0f * exitVelo * exitVelo);
  ...
  RackMotor.SetControl(ctre::phoenix6::controls::PositionDutyCycle{
      units::angle::turn_t{targetAbs}});
}

float Shooter::findOptimalRPM(float horizontalOffset, float yOffset) {
  float dx = (1.0f) +
             std::sqrt(horizontalOffset * horizontalOffset + yOffset * yOffset);
  ...
  if (dx < kTable.front().dx)
    return 0.0f;
  if (dx >= kTable.back().dx)
    return kTable.back().effectiveRPM;
  ...
  for (size_t i = 0; i + 1 < kTable.size(); i++) {
    ...
    if (dx >= kTable[i].dx && dx < kTable[i + 1].dx) {
      ...
      float effRPM = kTable[i].effectiveRPM +
                     t * (kTable[i + 1].effectiveRPM - kTable[i].effectiveRPM);
      return effRPM;
    }
  }
  return 0.0f;
...
void Shooter::setManualHoodPosition(float targetAngleDeg) {
  ...
  // deltaDeg is relative to the resting/max angle (68°), matching
  // setHoodPosition's convention
  float deltaDeg = targetAngleDeg - 68.0f;
  double motorTurnsTarget =
      (deltaDeg / 360.0) * ShooterConstants::motorGearRatio;
  double targetAbsLocal = hoodCenterRot + motorTurnsTarget;
...
double Shooter::findHoodAngle() {
  // Inverse of setHoodPosition's motor-turns formula: angle = turns/gearRatio *
  // 360 + restingAngle
  return (RackMotor.GetPosition().GetValueAsDouble() /
          ShooterConstants::motorGearRatio) *
             360.0 +
         68.0;
}
