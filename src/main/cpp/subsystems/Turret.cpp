...
void Turret::init() {
  ...
  // turretConfigs.MagnetSensor.SensorDirection =
  // ctre::phoenix6::signals::SensorDirectionValue::CounterClockwise_Positive;
  turretConfigs.MotorOutput.Inverted =
      ctre::phoenix6::signals::InvertedValue::CounterClockwise_Positive;
  turretConfigs.MotorOutput.NeutralMode =
...
double Turret::getCurrentMotorAngle() {
  double motorPos = turretMotor.GetPosition().GetValueAsDouble() *
                    360;  // find pos in degrees
  double currentAngle = std::fmod(motorPos / TurretConstants::turretPulleyRatio,
                                  360.0);  // pullyRatio = 44
...
// Not yet tested; We can get the relative value from the encoder, which makes
// life easier
double Turret::getCurrentEncoderAngle() {
  double encoderPos =
      encoder.GetPosition().GetValueAsDouble() * 360;  // find pos in degrees
  double currentAngle = std::fmod(encoderPos / TurretConstants::turretTbRatio,
                                  360.0);  // tb to turret ratio = 8.7777
  if (currentAngle > 180.0)
    currentAngle -= 360.0;
  if (currentAngle < -180.0)
    currentAngle += 360.0;
...
void Turret::autoMoveToTarget() {
  ...
  double robotX_in =
      mDrive.getPose().X().value() * ShooterConstants::MeterToInches;
  double robotY_in =
      mDrive.getPose().Y().value() * ShooterConstants::MeterToInches;
  double dx = TurretConstants::hubX - robotX_in;
  double dy = TurretConstants::hubY - robotY_in;

  // atan2(dx, dy) gives angle from +Y (forward) axis, CW-positive toward +X
  // (right). Negate to make CCW-positive so it matches the robotHeading
  // convention (0 = facing forward/+Y, CCW+).
  ...
  while (turretTarget > 180)
    turretTarget -= 360;
  while (turretTarget < -180)
    turretTarget += 360;

  double clampedTarget =
      std::clamp(turretTarget, -TurretConstants::turretMaxAngle,
                 TurretConstants::turretMaxAngle);

  frc::SmartDashboard::PutNumber("turret angle", turretTarget);
  frc::SmartDashboard::PutBoolean("is angle in range?",
                                  turretTarget == clampedTarget);

  turretMotor.SetControl(
      positionVoltage
          .WithPosition(units::angle::turn_t(
              clampedTarget / 360 * TurretConstants::turretPulleyRatio))
          .WithSlot(0));
}

void Turret::setAngle(double targetAngle) {
  double clampedTarget =
      std::clamp(targetAngle, -TurretConstants::turretMaxAngle,
                 TurretConstants::turretMaxAngle);
  turretMotor.SetControl(
      positionVoltage
          .WithPosition(units::angle::turn_t(
              targetAngle / 360 * TurretConstants::turretPulleyRatio))
          .WithSlot(0));
}
...
bool Turret::isAtAngle(double targetAngle) {
  ...
  if (std::fabs(targetAngle - actualEncVal) < 2)
    return true;  // if the current angle is within 5 degrees of the target
                  // angle
  else
    return false;
}
...
void Turret::autoTarget() {
    double heading =
QuestNav::getInstance().getPose2d().Rotation().Degrees().value(); double
targetDeg = (heading - TurretConstants::hubHeading); setAngle(targetDeg);
}
*/
