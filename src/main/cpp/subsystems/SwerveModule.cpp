...
void SwerveModule::setMotor(control controlType, MotorType motorType,
  ...
  switch (controlType) {
        ...
        driveMotor.SetControl(
            velocityVoltage
                .WithVelocity(units::turns_per_second_t(
                    input / (0.1016 * M_PI) * ModuleConstants::kDriveGearRatio))
                .WithSlot(0));
      } else {
    ...
    case control::POSITION:
      if (motorType == MotorType::DRIVE) {
        driveMotor.SetControl(
            positionVoltage.WithPosition(units::turn_t(input)).WithSlot(0));
      } else {
        steerMotor.SetControl(
            positionVoltage.WithPosition(units::turn_t(input)).WithSlot(0));
      }
...
void SwerveModule::initHardware() {
  // Drive Configs
  driveConfigs.Slot0.kP =
      0.35;  // An error of 1 rotation per second results in [] output
  driveConfigs.Slot0.kI = 0.0;  // An error of 1 rotation per second increases
                                // output by [] every second
  driveConfigs.Slot0.kD = 0.0;  // A change of 1 rotation per second squared
                                // results in []] volts output
  driveConfigs.Slot0.kV =
      0.0;  // Falcon 500 is a 500kV motor, 500rpm per V = 8.333 rps per V,
  ...
  driveConfigs.MotorOutput.NeutralMode =
  ...
  // Steer Configs
  steerConfigs.Slot0.kP =
      0.9;  // An error of 1 rotation per second results in [] output
  steerConfigs.Slot0.kI = 0.0;  // An error of 1 rotation per second increases
                                // output by [] every second
  steerConfigs.Slot0.kD = 0.0;  // A change of 1 rotation per second squared
                                // results in []] volts output
  steerConfigs.Slot0.kV =
      0.0;  // Falcon 500 is a 500kV motor, 500rpm per V = 8.333 rps per V,
            // 1/8.33 = 0.12 volts / Rotation per second
  ...
  steerConfigs.MotorOutput.NeutralMode =
      ctre::phoenix6::signals::NeutralModeValue::Brake;
  ...
  // Encoder
  encoderConfigs.MagnetSensor.WithSensorDirection(
...
void SwerveModule::invertModule(ctre::phoenix6::signals::InvertedValue value,
                                bool steer, bool drive) {
  if (steer) {
...
void SwerveModule::setDesiredState(frc::SwerveModuleState& state) {
  auto optimizedState = frc::SwerveModuleState::Optimize(
      state, units::radian_t(encoder.GetAbsolutePosition().GetValueAsDouble() *
                             MathConstants::TWO_PI));
  frc::SmartDashboard::PutNumber(
      "encoder pos", encoder.GetAbsolutePosition().GetValueAsDouble());

  setMotor(VELOCITY, DRIVE, optimizedState.speed.value());
  setMotor(POSITION, STEER,
           ((optimizedState.angle.Radians().value()) / (2 * M_PI) *
            ModuleConstants::kSteerGearRatio));
}

frc::SwerveModulePosition SwerveModule::getPosition() {
  double conversionFactor = M_PI * 0.1016 / ModuleConstants::kDriveGearRatio;
  return frc::SwerveModulePosition{
      units::meter_t(driveMotor.GetPosition().GetValueAsDouble() *
                     conversionFactor),
      units::radian_t(encoder.GetAbsolutePosition().GetValueAsDouble() *
                          MathConstants::TWO_PI -
                      moduleOffset)};
}
